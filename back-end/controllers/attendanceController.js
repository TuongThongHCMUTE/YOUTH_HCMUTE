...
exports.getAllAttendances = async (req, res, next) => {
    try {
        ...
        res.status(200).json({
            ...
            results: limit ? limit : event.sinhViens.length,
            data: {
...
exports.modifyAttendance = async (req, res, next) => {
    try {
        ...
        if (student) {
            let event = await Event.findById(id).select('soLuongSinhVienDangKy soLuongSinhVien').select({ sinhViens: {$elemMatch: {maSoSV}}})
